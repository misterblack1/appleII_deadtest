...
start:	LDA #$00	; clear accum
        LDX #$15	; index into test value table
        LDY #$00	; start test at offset 0

zp_wr:	LDA tst_tbl,X   ; fetch test value into A
        STA $0000,Y		; write offset Y in each page of first 4KB
        STA $0100,Y
        STA $0200,Y
        STA $0300,Y
        STA $0400,Y
        ...
        STA $0E00,Y
        STA $0F00,Y
        INY 			; next address increasing
        BNE zp_wr		; loop to fill the pages
        TXA 			; save test value index into A

        LDX #$00		; clear X and Y regs for the delay loop
        LDY #$00
zpw_p:	DEY 			; wait a bit - detect refresh errors, where cells may decay - wait at least 100ms
        BNE zpw_p
        DEX 
        BNE zpw_p
        TAX 
		
zp_rd:	LDA $0000,Y			; now checking to see if the contents of RAM is still good
        CMP tst_tbl,X		; compare A to the test pattern table entry
        BNE biterr
		LDA $0100,Y
        CMP tst_tbl,X
        BNE biterr
...
noerr:	DEX 			; no error, so look at the next test value
        BMI ramgood
        LDY #$00
        JMP zp_wr

ramgood:; memtest ok put the RAM test good code here
